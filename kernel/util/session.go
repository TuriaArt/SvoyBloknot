...
// IsBrowserRequest 判断请求是否来自浏览器（非 SvoyBloknot 原生客户端）。
// 原生客户端（桌面 Electron、Android/iOS/Harmony）的 User-Agent 均以 "SvoyBloknot/" 开头，
// 其余视为浏览器。该口径与前端 getFrontend()、electron/main.js 设置的 UA 前缀、
// 以及 session 鉴权中既有的 HasPrefix(ua, "SvoyBloknot/") 判断保持一致。
func IsBrowserRequest(c *gin.Context) bool {
	return !strings.HasPrefix(c.GetHeader("User-Agent"), "SvoyBloknot/")
}
